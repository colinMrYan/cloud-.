...
public class ContactUser {
    ...
    public ContactUser(String id, String name, String nameGlobal, String pinyin, String parentId, String mobile, String email, int hasHead, int sortOrder) {
        this.id = id;
        this.name = name;
    ...
    public static List<ContactUser> protoBufUserList2ContactUserList(List<ContactProtoBuf.user> userList){
        List<ContactUser> contactUserList = new ArrayList<>();
        if (userList != null && userList.size()>0){
            int size = userList.size();
            for (int i=0;i<size;i++){
                ContactProtoBuf.user user = userList.get(i);
                ContactUser contactUser = new ContactUser(user.getId(),user.getRealName(),user.getNameGlobal(),user.getPinyin(),user.getParentId(),user.getMobile(),user.getEmail(),user.getHasHead(),user.getSortOrder());
