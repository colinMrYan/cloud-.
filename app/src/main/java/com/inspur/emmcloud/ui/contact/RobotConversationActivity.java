 ...
 */
public class RobotConversationActivity extends BaseActivity implements OnStateChangedListener{
	...
	private void initViews() {
		apiService = new ContactAPIService(RobotConversationActivity.this);
		apiService.setAPIInterface(new WebService());
		loadingDialog = new LoadingDialog(RobotConversationActivity.this);
	...
	private void getRobotInfo() {
		Robot robotInfo = RobotCacheUtils.getRobotById(RobotConversationActivity.this, id);
		if(robotInfo == null){
			getRobotInfoFromNet();
		}else {
	...
	private void getRobotInfoFromNet() {
		if(NetUtils.isNetworkConnected(RobotConversationActivity.this)){
			loadingDialog.show();
	...
	private void setChannelTop(boolean isSetIop) {
		...
		ChannelOperationCacheUtils.setChannelTop(RobotConversationActivity.this, cid,
				isSetIop);
		// 通知消息页面重新创建群组头像
	...
	class WebService extends APIInterfaceInstance{
		...
		public void returnRobotByIdFail(String error,int errorCode) {
			...
			WebServiceMiddleUtils.hand(RobotConversationActivity.this, error,errorCode);
		}
	}
