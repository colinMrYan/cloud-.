 ...
 */
package com.inspur.emmcloud.widget.mylistview;
...
public class MyListView extends ListView implements OnScrollListener {
	...
	private boolean isSlidingDelete;
	...
	private void initWithContext(Context context) {
		...
		headerHintTextview = (TextView)mHeaderView.findViewById(R.id.xlistview_header_hint_textview);
		refreshTimeLayout = (LinearLayout)mHeaderView.findViewById(R.id.refresh_time_layout);
		addHeaderView(mHeaderView,null,false);   //添加参数false，防止header被点击报错
	...
	public void setLoadHistory(boolean isLoadHistory){
		headerHintTextview.setText(context.getString(R.string.pull_down_load_history));
		refreshTimeLayout.setVisibility(View.GONE);
		mHeaderView.setLoadHistory(true);
	}
	...
	public void  setSlidingDelete(boolean isSlidingDelete) {
		this.isSlidingDelete = isSlidingDelete;
	}
...
//		if (isSlidingDelete && !(mHeaderView.getVisiableHeight() > 0) &&!(mFooterView !=null && mFooterView.getBottomMargin() > 0)) {
//			switch (ev.getAction()) {
...
//	            if (!(position == INVALID_POSITION ||(mEnablePullRefresh && position == 0) ||(mEnablePullLoad && position == (this.getCount()-1)))) {
////	            	ListSlideItem data = (ListSlideItem) getItemAtPosition(position);
...
//	        	if (mFocusedItemView.onRequireTouchEvent(ev)) {
//	        		return true;
//				}
//	        }
//		}
		...
		case MotionEvent.ACTION_MOVE:
			...
			if (getFirstVisiblePosition() == 0
				...
				flag = 1;
			} else if (getLastVisiblePosition() == mTotalItemCount - 1 && (mFooterView != null)
		...
		default:
			mLastY = -1; // reset
			if (getFirstVisiblePosition() == 0 && flag == 1) {
				// invoke refresh
