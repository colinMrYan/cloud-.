...
import com.inspur.emmcloud.util.common.richtext.drawable.DrawableSizeHolder;
...
import com.inspur.emmcloud.util.common.richtext.ext.TextKit;
...
public class DefaultImageGetter implements ImageGetter, ImageLoadNotify {
    ...
    private static final int TASK_TAG = R.drawable.icon_default;
        ...
        try {
            ...
            if (config.cacheType.intValue() > CacheType.layout.intValue()) {
                ...
                if (bitmap != null) {
                    ...
                    Debug.log(TAG, "cache hit -- memory > " + holder.getSource());
                    ...
                    Debug.log(TAG, "cache hit -- local > " + holder.getSource());
                }
            }
            if (imageLoader == null) {
                Debug.log(TAG, "cache hit -- none");
                // 无缓存图片，直接加载
                if (Base64.isBase64(holder.getSource())) {
                    ...
                    Debug.log(TAG, "image load -- base64 > " + holder.getSource());
                    ...
                    Debug.log(TAG, "image load -- asset > " + holder.getSource());
                    ...
                    Debug.log(TAG, "image load -- local > " + holder.getSource());
                } else {
                    // 网络图片
                    ...
                    Debug.log(TAG, "image load -- remote > " + holder.getSource());
                }
            }
        } catch (Exception e) {
