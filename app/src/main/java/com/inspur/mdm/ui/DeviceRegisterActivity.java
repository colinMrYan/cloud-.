...
public class DeviceRegisterActivity extends MDMBaseActivity {
	...
	private void initDeviceRegisterUI() {
		...
		deviceNameEdit.setText(userName
				+ getResources().getString(
						(isPad ? MDMResUtils.getStringID("_pad") : MDMResUtils
								.getStringID("_mobile"))));
		...
		if (requireFieldList.contains("device_name")) {
			...
			((ImageView) findViewById(MDMResUtils
		...
		if (requireFieldList.contains("phone_number")) {
			isPhoneNumNeed = true;
			((ImageView) findViewById(MDMResUtils.getWidgetID("phone_num_img")))
					.setVisibility(View.VISIBLE);
		}

		if (requireFieldList.contains("email")) {
			isEmailNeed = true;
			((ImageView) findViewById(MDMResUtils.getWidgetID("mail_img")))
					.setVisibility(View.VISIBLE);
		}

		if (requireFieldList.contains("remark")) {
			isRemarkNeed = true;
			((ImageView) findViewById(MDMResUtils.getWidgetID("remark_img")))
					.setVisibility(View.VISIBLE);
		}
	...
	public void onClick(View v) {
		if (v.getId() == MDMResUtils.getWidgetID("regesiter_btn")) {
			...
			if (isEmailNeed && !MDMUtils.isMobileNum(phoneNum)) {
				Toast.makeText(getApplicationContext(),
						MDMResUtils.getStringID("telephone_input_illegal"),
						Toast.LENGTH_SHORT).show();
				return;
			}
			if (isEmailNeed && !MDMUtils.isEmail(mail)) {
				Toast.makeText(getApplicationContext(),
