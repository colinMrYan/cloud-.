package com.inspur.emmcloud.baselib.widget.dataTimePickers;
...
import com.inspur.emmcloud.baselib.widget.dataTimePickers.dataTimePickerUtils.DateUtils;
...
public class DateTimePicker extends WheelPicker {
    ...
    private boolean haveMinInterval = false;
    ...
    public void setIntervalMinutes(boolean haveMinInterval) {
        this.haveMinInterval = haveMinInterval;
    }
    ...
    protected View makeCenterView() {
        ...
        if ((dateMode == YEAR_MONTH_DAY || dateMode == YEAR_MONTH) && years.size() == 0) {
            LogUtils.LbcDebug("init years before make view");
            initYearData();
        }
        if (dateMode != NONE && months.size() == 0) {
            LogUtils.LbcDebug("init months before make view");
        ...
        if ((dateMode == YEAR_MONTH_DAY || dateMode == MONTH_DAY) && days.size() == 0) {
            LogUtils.LbcDebug("init days before make view");
            int selectedYear;
            if (dateMode == YEAR_MONTH_DAY) {
        ...
        if (timeMode != NONE && hours.size() == 0) {
            LogUtils.LbcDebug("init hours before make view");
            initHourData();
        }
        if (timeMode != NONE && minutes.size() == 0) {
            LogUtils.LbcDebug("init minutes before make view");
        ...
        if (dateMode == YEAR_MONTH_DAY || dateMode == YEAR_MONTH) {
            ...
            yearView.setOnItemSelectListener(new WheelView.OnItemSelectListener() {
                ...
                public void onSelected(int index) {
                    ...
                    LogUtils.LbcDebug("change months after year wheeled");
                    if (resetWhileWheel) {
        ...
        if (dateMode != NONE) {
            ...
            monthView.setOnItemSelectListener(new WheelView.OnItemSelectListener() {
                ...
                public void onSelected(int index) {
                    ...
                    if (dateMode == YEAR_MONTH_DAY || dateMode == MONTH_DAY) {
                        LogUtils.LbcDebug("change days after month wheeled");
                        if (resetWhileWheel) {
                            selectedDayIndex = 0;//重置日子索引
                        }
                        int selectedYear;
        ...
        if (timeMode != NONE) {
            ...
            hourView.setOnItemSelectListener(new WheelView.OnItemSelectListener() {
                ...
                public void onSelected(int index) {
                    ...
                    LogUtils.LbcDebug("change minutes after hour wheeled");
                    changeMinuteData(DateUtils.trimZero(selectedHour));
    ...
    private void changeMonthData(int selectedYear) {
        ...
        if (!resetWhileWheel) {
            ...
            LogUtils.LbcDebug("preSelectMonth=" + preSelectMonth);
        }
        months.clear();
    ...
    private void changeDayData(int selectedYear, int selectedMonth) {
        ...
        if (!resetWhileWheel) {
            ...
            LogUtils.LbcDebug("maxDays=" + maxDays + ", preSelectDay=" + preSelectDay);
        }
        days.clear();
    ...
    private void initHourData() {
        ...
        if (!resetWhileWheel) {
            if (haveMinInterval) {
                ...
                if (minutes % 15 != 0) {
                    selectedMinute = DateUtils.fillZero(((minutes / 15) + 1) * 15);
                } else {
