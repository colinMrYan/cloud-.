 ...
 * desc   : Toast 工具类
 */
public final class CusToastUtils {
     ...
     * 不允许外部实例化
     */
    private CusToastUtils() {
    }

    /**
     * 初始化 CusToastUtils 及样式
     */
    ...
    /**
     * 初始化 CusToastUtils，在Application中初始化
     *
    ...
    public static void initStyle(IToastStyle style) {
        CusToastUtils.sDefaultStyle = style;
        // 如果吐司已经创建，就重新初始化吐司
        if (sToast != null) {
            // 取消原有吐司的显示
            sToast.cancel();
    ...
    /**
     * 检查吐司状态，如果未初始化请先调用{@link CusToastUtils#init(Application)}
     */
    private static void checkToastState() {
        // 吐司工具类还没有被初始化，必须要先调用init方法进行初始化
        if (sToast == null) {
            throw new IllegalStateException("CusToastUtils has not been initialized");
        }
    }
