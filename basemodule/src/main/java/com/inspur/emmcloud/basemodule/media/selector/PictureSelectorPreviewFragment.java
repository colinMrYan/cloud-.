...
import com.inspur.emmcloud.basemodule.media.selector.adapter.holder.PreviewGalleryAdapter;
...
import com.inspur.emmcloud.basemodule.media.selector.basic.PictureMediaScannerConnection;
import com.inspur.emmcloud.basemodule.media.selector.config.Crop;
...
import com.inspur.emmcloud.basemodule.media.selector.decoration.HorizontalItemDecoration;
import com.inspur.emmcloud.basemodule.media.selector.decoration.WrapContentLinearLayoutManager;
import com.inspur.emmcloud.basemodule.media.selector.dialog.PictureCommonDialog;
...
import com.inspur.emmcloud.basemodule.media.selector.loader.LocalMediaLoader;
import com.inspur.emmcloud.basemodule.media.selector.loader.LocalMediaPageLoader;
...
import com.inspur.emmcloud.basemodule.media.selector.utils.DownloadFileUtils;
...
import com.inspur.emmcloud.basemodule.media.selector.widget.BottomNavBar;
import com.inspur.emmcloud.basemodule.media.selector.widget.CompleteSelectView;
...
//    protected PicturePreviewAdapter viewPageAdapter;
    ...
    // support版本 预览adapter
    private PicturePreviewSupportAdapter previewAdapter;
    ...
    protected void setMagicalViewAction() {
        magicalView.setOnMojitoViewCallback(new OnMagicalViewCallback() {
            ...
            public void onBeginBackMinAnim() {
                // 设置图片/播放按钮 visible
                PreviewSupportHolder currentHolder = previewAdapter.getCurrentHolder(viewPager.getCurrentItem());
                if (currentHolder == null) {
                    return;
                }
                if (currentHolder.photoView.getVisibility() == View.GONE) {
                    currentHolder.photoView.setVisibility(View.VISIBLE);
                }
                if (!currentHolder.isPicType) {
                    if (currentHolder.playIv.getVisibility() == View.VISIBLE) {
                        currentHolder.playIv.setVisibility(View.GONE);
                    }
                }
            }
            ...
            public void onBeginMagicalAnimComplete(MagicalView mojitoView, boolean showImmediately) {
                ...
                // ViewPager 设置photoView scaleType
                PreviewSupportHolder currentHolder = previewAdapter.getCurrentHolder(viewPager.getCurrentItem());
                if (currentHolder == null) {
                    return;
                }
                ...
                if (MediaUtils.isLongImage(realWidth, realHeight)) {
                    currentHolder.photoView.setScaleType(ImageView.ScaleType.CENTER_CROP);
                } else {
                    currentHolder.photoView.setScaleType(ImageView.ScaleType.FIT_CENTER);
                }
                if (!currentHolder.isPicType) {
                    if (currentHolder.playIv.getVisibility() == View.GONE) {
                        currentHolder.playIv.setVisibility(View.VISIBLE);
                    }
                }
            }
            ...
            public void onBeginBackMinMagicalFinish(boolean isResetSize) {
                ...
                // ViewPager 设置item visible
                PreviewSupportHolder currentHolder = previewAdapter.getCurrentHolder(viewPager.getCurrentItem());
                if (currentHolder == null) {
                    return;
                }
                currentHolder.photoView.getLayoutParams().width = itemViewParams.width;
                currentHolder.photoView.getLayoutParams().height = itemViewParams.height;
                currentHolder.photoView.setScaleType(ImageView.ScaleType.CENTER_CROP);
            }
        });
    }
...
//                                    viewPageAdapter.setVideoPlayButtonUI(newPosition);
    ...
    public PicturePreviewSupportAdapter getAdapter() {
...
//        viewPager.setPageTransformer(new MarginPageTransformer(DensityUtil.dip2px(getContext(), 3)));
    ...
    private final ViewPager.OnPageChangeListener pageChangeCallback = new ViewPager.OnPageChangeListener() {
        @Override
...
//                                    || position == previewAdapter.getCount() - 1) {
//                                loadMoreData();
//                            }
//                        }
//                    }
...
//    }
    ...
    private void changeMagicalViewParams(final int position) {
        final LocalMedia media = mData.get(position);
        final int[] size = getRealSizeFromMedia(media);
