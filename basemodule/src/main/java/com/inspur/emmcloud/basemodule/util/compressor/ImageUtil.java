...
class ImageUtil {
    ...
    static File compressImage(File imageFile, int reqArea, Bitmap.CompressFormat compressFormat, int quality, String destinationPath) throws IOException {
        FileOutputStream fileOutputStream = null;
        File file = new File(destinationPath).getParentFile();
        if (!file.exists()) {
            file.mkdirs();
        ...
        try {
            // write the compressed bitmap at the destination specified by destinationPath.
            Bitmap bitmap = decodeSampledBitmapFromFile(imageFile, reqArea);
            File outFile = new File(destinationPath);
            if (outFile.exists()) {
                outFile.delete();
            }
    ...
    static Bitmap decodeSampledBitmapFromFile(File imageFile, int reqArea) throws IOException {
        // First decode with inJustDecodeBounds=true to check dimensions
        BitmapFactory.Options options = new BitmapFactory.Options();
        options.inJustDecodeBounds = true;
        options.inPreferredConfig = Bitmap.Config.RGB_565;
        ...
        // Calculate inSampleSize
        options.inSampleSize = calculateInSampleSize(options, reqArea);
        ...
        int area = scaledBitmap.getWidth()*scaledBitmap.getHeight();
        if (area > reqArea) {
            float scaleSum = (float) reqArea / area;
            float  scale = (float)Math.sqrt(scaleSum);
            //宽高等比例缩放
            matrix.postScale(scale, scale);
        }
        scaledBitmap = Bitmap.createBitmap(scaledBitmap, 0, 0, scaledBitmap.getWidth(), scaledBitmap.getHeight(), matrix, true);
    ...
    private static int calculateInSampleSize(BitmapFactory.Options options, int reqArea) {
        // Raw height and width of image
        final int height = options.outHeight;
        final int width = options.outWidth;
        int inSampleSize = 1;
        ...
        if (height*width > reqArea) {
            //因为涉及到面积，面积缩小的倍数为长缩小倍数*宽缩小倍数
            while ((height*width / Math.pow(inSampleSize,2)) >= reqArea ) {
                inSampleSize *= 2;
            }
            //当bitmap压缩到最后一次时能用Matrix就用
            if (inSampleSize >= 2 && (height*width / Math.pow(inSampleSize,2))*4 < Math.pow(MyAppConfig.UPLOAD_ORIGIN_IMG_MAX_SIZE,2) ) {
                inSampleSize = inSampleSize / 2;
            }
        }
        LogUtils.jasonDebug("inSampleSize=" + inSampleSize);
